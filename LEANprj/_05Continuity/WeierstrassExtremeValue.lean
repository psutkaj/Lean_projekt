...
theorem WeierstrassBdd (ax_NIP : AxNIP)
  {f : ℝ → ℝ} {a b : ℝ}
  (h_cont : FunContinuousOnSet (Set.Icc a b) f) :
  FunBddOnSet (Set.Icc a b) f :=
by
  by_contra h_unbdd
  unfold FunBddOnSet at h_unbdd
  push_neg at h_unbdd
  ...
  have hc_in_Int : c ∈ Set.Icc a b := by
    ...
    · by_contra hnot
      ...
      linarith
  have h_cont_at_c : FunContinuousAt f c := h_cont c hc_in_Int
  have h_f_conv : ConvergesTo (f ∘ (x_seq ∘ k)) (f c) := by
    ...
    · apply hδ
      ...
      · exact hN
  have : Convergent (f ∘ (x_seq ∘ k))
  · use f c
  obtain ⟨K, K_pos, hK⟩ := bdd_of_conv this
  obtain ⟨n, hn⟩ := exists_nat_gt K
  have f_le_n : |(f ∘ x_seq ∘ k) n| < n
  · calc |(f ∘ x_seq ∘ k) n|
    _ ≤ K := hK n
    _ < n := hn
  have  f_ge_kn := (h_x_seq_prop (k n)).2
  have hkn: k n ≥ n := StrictlyIncreasingSequenceN_ge_id hk_inc n
  have : |f (x_seq (k n))| ≥ n + 1
  · calc |f (x_seq (k n))|
    _ ≥ k n + 1 := f_ge_kn
    _ ≥ n + 1 := by
      simp only [ge_iff_le, add_le_add_iff_right, Nat.cast_le];
      exact hkn
  simp only [Function.comp_apply] at f_le_n
  linarith

theorem ContinuousImpliesSequential {f : ℝ → ℝ} {c : ℝ} (h_cont : FunContinuousAt f c)
...
by
  ...
  have S_bdd : BoundedSet S := by
    obtain ⟨c, c_pos, hc⟩ := WeierstrassBdd ax_NIP h_cont
    use c, c_pos
    intro m hm
    dsimp [S] at hm
  ...
  have S_upper_bdd : ∃ u : ℝ, ∀ a ∈ S, a ≤ u := by
    ...
    · exact le_abs_self a
    · exact hc
  obtain ⟨M, ⟨upper_bd, lowest_upper⟩, UniqueM⟩ := axSup_of_axNip ax_NIP S S_nonempty S_upper_bdd
  ...
  obtain ⟨k, hk_inc, ⟨c, hc⟩⟩ := axBw_of_axMonoConv (axMonoConv_of_axSup (axSup_of_axNip ax_NIP)) x x_bdd
  ...
  have h_lim_construction : ConvergesTo (f ∘ (Subsequence x k)) M := by
    ...
    · have h_calc : f (x (k n)) < M + ε := calc f (x (k n))
      ...
      dsimp [Subsequence]
      linarith
  have M_eq_fc : M = f c := convergesTo_unique h_lim_construction h_lim_continuity
  rw [← M_eq_fc]
  apply upper_bd
  use d, d_in_Int
...
by
  ...
  have g_cont : FunContinuousOnSet (Set.Icc a b) g := by
    ...
    rw [←this]
    exact hδ
  obtain ⟨m, hm, m_leq⟩ := WeierstrassMax ax_NIP hab g_cont
  use m, hm
  intro x hx
