...
lemma ContinuityKeepsSgnNeg {f : ℝ → ℝ} {x₀ : ℝ} (h_cont : FunContinuousAt f x₀) (h_val : f x₀ < 0) :
  ∃ δ > 0, ∀ x, |x - x₀| < δ → f x < 0 :=
by
  let ε := - f x₀
...
lemma ContinuityKeepsSgnPos {f : ℝ → ℝ} {x₀ : ℝ} (h_cont : FunContinuousAt f x₀) (h_val : f x₀ > 0) :
  ∃ δ > 0, ∀ x, |x - x₀| < δ → f x > 0 :=
by
  let ε := f x₀
